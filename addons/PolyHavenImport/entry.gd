...
func _ready() -> void:
	...
	add_child(download)
	download.downloading.connect(download_progress)
	download.downloaded.connect(download_completed)
...
func populate_quality_drop_down():
	...
	qualities.sort_custom(func(a, b): return a.to_int() > b.to_int())
...
func _import_resource(path: String) -> void:
...
func import(results:Array):
	...
	if info["type"] == 0: # HDRI
		...
		info.file_path=resourcepath.path_join(filen)
		f = FileAccess.open(info.file_path, FileAccess.WRITE)
		f.store_buffer(results[0].result)
		f.close()

		EditorInterface.get_resource_filesystem().filesystem_changed.connect(save_resource,CONNECT_ONE_SHOT)
		api._rescan_files()
		...
		for result in results:
			...
			if filen.ends_with(".jpg") or filen.ends_with(".jpeg") or filen.ends_with(".png") or filen.ends_with(".exr"):
				...
				var filen_path=resourcepath.path_join(filen)
				if "_diff_" in filen.to_lower():
					textures["albedo"] = filen_path
				if "_metal_" in filen.to_lower():
					textures["metallic"] = filen_path
...
func save_resource():
	if  not info.has("resourcepath"):
		return
	if info["type"] == 0: # HDRI
		var resource = Sky.new()
		var panorama = PanoramaSkyMaterial.new()
		panorama.set_panorama(load(info.file_path))
		...
		var quality = QualityDropDown.get_item_text(QualityDropDown.selected)
		var res_path:String = info.resourcepath.path_join("textures").path_join(asset_name+".tres")
		ResourceSaver.save(resource, res_path, ResourceSaver.FLAG_CHANGE_PATH)
		pass
	elif info["type"] == 2: # Model
		var resource := StandardMaterial3D.new()
		if  textures.albedo:
